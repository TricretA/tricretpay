...
app.post('/api/stkpush', async (req, res) => {
  ...
  try {
    ...
    const payload = {
      ...
      "CallBackURL": process.env.NGROK_URL + '/api/callback', // must be publicly accessible
  ...
  try {
    ...
    if(checkoutRequestID && payments[checkoutRequestID]){
      if(resultCode === 0){
        ...
        console.log('Payment failed or cancelled for CheckoutRequestID:', checkoutRequestID, 'ResultCode:', resultCode);
...
app.get('/api/status', (req,res) => {
  ...
  console.log('Status request - CheckoutRequestID:', id, 'Entry:', entry);
  if(!entry) {
